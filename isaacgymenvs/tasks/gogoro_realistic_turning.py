...
class Gogoro(VecTask):
    ...
    def _create_ground_plane(self):
        ...
        env_spacing = 2
        envs_scale  = env_spacing * int(np.sqrt(self.num_envs))
    ...
    def post_physics_step(self):
        ...
        if(PUSH_ROBOT):
            ...
            push_force = 10.0
            curent_yaw = curent_yaw[need_update]


            x_force = ((torch.rand(self.num_envs,device=self.device)*2-1)*push_force)[need_update]
            z_force = -((torch.rand(self.num_envs,device=self.device))*push_force)[need_update]


            self.curent_perturbations[need_update,self.rgid_shape_to_id["head_p_link"],0] =  x_force*torch.cos(curent_yaw+torch.pi/2)
            self.curent_perturbations[need_update,self.rgid_shape_to_id["head_p_link"],1] =  x_force*torch.sin(curent_yaw+torch.pi/2)
            self.curent_perturbations[need_update,self.rgid_shape_to_id["head_p_link"],2] =  z_force
            ...
            xpert = self.curent_perturbations[test,self.rgid_shape_to_id["head_p_link"],0]
            ypert = self.curent_perturbations[test,self.rgid_shape_to_id["head_p_link"],1]
            zpert = self.curent_perturbations[test,self.rgid_shape_to_id["head_p_link"],2]
    ...
    def compute_obs_rwd(self):
        ...
        #print(self.buffer_obs_noisy)
        # print("curent_speed =",self.buffer_obs[:,-1,4])
        # print("input  speed =",self.buffer_obs_noisy[:,-1,4]*5.0)
        # print("offset       =",self.curent_speed_offset)

        #REMOVING THE ACCEL
        #self.buffer_obs_noisy[:,:,2:4] = 0.0
        #REMOVING YAW
        self.buffer_obs_noisy[:,:,1] = 0.0
        #self.buffer_obs_noisy[:,:,3] = 0.0
    ...
    def generate_spawn_r(self,n_envs):
        ...
        initial_rot     = initial_target +  self.get_randoms(n_envs,[-np.pi,np.pi]) #(torch.rand(n_envs,device=self.device)*2-1)*np.pi #initial_target
    ...
    def reset_idx(self, env_ids):
        ...
        if(DEBUGUSETERRAIN):
            ...
            sqrt_root   = int(np.sqrt(self.num_envs))
            env_spacing = 2
            envs_scale  = env_spacing * sqrt_root
            start_mid   = int(self.terrain.Vx_size_m/2 - envs_scale/2)
            start_mid_index = int(start_mid * (1/self.terrain.V_scale))

            test[env_ids_int32,2] += self.terrain.heightsamples[start_mid_index+((env_ids %sqrt_root) * (env_spacing/self.terrain.V_scale)).to(torch.int) ,
                                                                start_mid_index+((env_ids//sqrt_root) * (env_spacing/self.terrain.V_scale)).to(torch.int)] * self.terrain.H_scale
...
def compute_gogoro_reward(act_buff,curent_obs, progress_buf, max_episode_length, yaw_update_freq):
    ...
    reward1 = 1/(1+yaw_err**2*200) #1-yaw_err**2#1-torch.abs(yaw_err) #1/(1+yaw_err**2*200)
    reward2 = 1/(1+tilt_err**2*100) #1-tilt_err**2
    reward4 = 1/(1+dtilt_err**2*100) #1-dtilt_err**2#torch.exp(-((tilt_err**2)*5.0)) * (1-yaw_err)

    min_ , _ =torch.min(act_buff, dim=1)
    max_ , _ =torch.min(act_buff, dim=1)
    action_err = min_-max_  
    reward3    = 1/(1+action_err**2*500)
    reward5    = 1-torch.abs(torch.mean(act_buff))
    
    reward     =  reward1*3.0 + reward2*1.0 + reward3*10.0 + reward4 * 2.0 + reward5 * 10.0 

 
 
     ## reward     = reward1*20.0 + reward2*1.0 + reward3*3.0 + reward4 * 1.0 + reward5 * 3.0  
    ## reward1*3.0 + reward2*0.1 + reward3*0.3 + reward4 * 0.0 + reward5 * 0.2
    ## reward     =  reward1*30.0 + reward2*1.0 + reward3*3.0 + reward4 * 1.0 + reward5 * 3.0 BEST
    #reward     =  reward1*3.0 + reward2*1.0 + reward3*3.0 + reward4 * 2.0 + reward5 * 3.0 
...
def compute_gogoro_observations(root_state,desired_yaw,curent_command, steer_delay):
    ...
    obs = torch.cat([
                        ...
                        # curent_damping_cfg.unsqueeze(-1),#7
                        steer_delay.unsqueeze(-1),#8
                        ], dim=1
                    )
