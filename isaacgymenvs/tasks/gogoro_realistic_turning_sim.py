...
class Gogoro(VecTask):
    ...
    def __init__(self, cfg, rl_device, sim_device, graphics_device_id, headless, virtual_screen_capture, force_render):
        ...
        #Sim Params
        self.n_envs             = (int)(cfg["env"]["numEnvs"]/2)
        ...
        self.command_history      = torch.zeros((self.n_envs*2,self.command_delay[1]),device=self.device)
        ...
        self.buffer_obs       = torch.zeros((self.n_envs*2,self.buff_size,num_obs),device=rl_device)
        self.buffer_obs_noisy = torch.zeros((self.n_envs*2,self.buff_size,num_obs),device=rl_device)
    ...
    def pre_physics_step(self, actions): 
        ...
        self.curent_command = (actions[:,0]*self.max_steering)*0.0
    ...
    def post_physics_step(self):
        ...
        if len(env_ids) > 0:
            self.reset_idx(env_ids[env_ids<2048])
        ...
        push_command_change  = (self.progress_buf==50).nonzero(as_tuple=False).flatten()


        if(speed_command_change.shape[0]!=0):
            speed_command_change = speed_command_change[speed_command_change<2048]
        if(yaw_command_change.shape[0]!=0):
            yaw_command_change   = yaw_command_change[yaw_command_change<2048]
        if(push_command_change.shape[0]!=0):
            push_command_change  = push_command_change[push_command_change<2048]
        ...
        if(PUSH_ROBOT):
            need_push  = (self.progress_buf[:2048]+1)%300 == 0
            push_force = 50.0
            curent_yaw = curent_yaw[:2048]


            x_force =  torch.zeros(self.n_envs,device=self.device)
            z_force = -torch.zeros(self.n_envs,device=self.device)

            x_force[need_push] = ((torch.rand(self.n_envs,device=self.device)*2-1)*push_force)[need_push]
            z_force[need_push] = -((torch.rand(self.n_envs,device=self.device))*push_force)[need_push]



            self.curent_perturbations[:,self.rgid_body_to_id["head_p_link"],0] =  x_force*torch.cos(curent_yaw+torch.pi/2)
            self.curent_perturbations[:,self.rgid_body_to_id["head_p_link"],1] =  x_force*torch.sin(curent_yaw+torch.pi/2)
            self.curent_perturbations[:,self.rgid_body_to_id["head_p_link"],2] =  z_force
    ...
    def compute_obs_rwd(self):
        ...
        self.buffer_obs_noisy[:2048,-1,0:2]  += self.get_randoms((self.n_envs,2),self.imu_filter_noise) #roll yaw
        self.buffer_obs_noisy[:2048,-1,0]    += self.curent_imu_x_offset
        self.buffer_obs_noisy[:2048,-1,2:4]  += self.get_randoms((self.n_envs,2),self.imu_noise)        #droll dyaw
        self.buffer_obs_noisy[:2048,-1,4]    += self.get_randoms(self.n_envs,self.speed_sensor_noise) #speed
        self.buffer_obs_noisy[:2048,-1,4]    += self.curent_speed_offset
        self.buffer_obs_noisy[:2048,-1,4]     = torch.where(self.buffer_obs_noisy[:2048,-1,4]<0,0,self.buffer_obs_noisy[:2048,-1,4])
        self.buffer_obs_noisy[:2048,-1,5]    += self.get_randoms(self.n_envs,self.imu_filter_noise) #delta yaw

        self.buffer_obs_noisy[2048:,-1,:]    = self.buffer_obs_noisy[:2048,-1,:].clone()


        assert torch.mean(self.buffer_obs_noisy[0+5,-1]) == torch.mean(self.buffer_obs_noisy[2048+5,-1])
        ...
        self.buffer_obs_noisy[2048:,-1,0]*=-1
        self.buffer_obs_noisy[2048:,-1,1]*=-1
        self.buffer_obs_noisy[2048:,-1,2]*=-1
        self.buffer_obs_noisy[2048:,-1,3]*=-1
        self.buffer_obs_noisy[2048:,-1,5]*=-1
        self.buffer_obs_noisy[2048:,-1,6]*=-1


        self.buffer_obs_noisy[:,:,1] = 0.0
        self.buffer_obs_noisy[:,:,4] = 0.0
    ...
    def generate_spawn_r(self,n_envs):
        initial_target  = (torch.rand(n_envs,device=self.device)*2-1)*np.pi * 0.0  #torch.zeros(n_envs,device=self.device) #(torch.rand(n_envs,device=self.device)*2-1)*np.pi
        initial_rot     = initial_target +  self.get_randoms(n_envs,[-np.pi,np.pi]) * 0.0  #(torch.rand(n_envs,device=self.device)*2-1)*np.pi #initial_target
        rot             = self.euler_to_quaternion((torch.rand(n_envs,device=self.device)*2-1)*0.1 * 0.0,
    ...
    def reset_idx(self, env_ids):
        ...
        self.progress_buf[env_ids_int32+2048]        = 0
        self.reset_buf[env_ids_int32+2048]           = 0
        self.obs_buf[env_ids_int32+2048]             = 0
        self.buffer_obs[env_ids_int32+2048]          = 0.0
        self.buffer_obs_noisy[env_ids_int32+2048]    = 0.0
        ...
        self.command_history[env_ids_int32+2048]     = 0.0
...
def compute_gogoro_reward(act_buff,curent_obs, progress_buf, max_episode_length, yaw_update_freq):
    ...
    reward6    = 1-((act_buff_[:2048,-1]- (-1*act_buff_[2048:,-1]))**2)
    reward6    = torch.cat((reward6,reward6))
    ...
    reward     = reward1*2.0 + reward2*0.1 + reward4 * 0.35 + reward5 * 2.0 + reward6 * 3.0 + reward7 * 0.2
...
def compute_gogoro_observations(root_state,desired_yaw,curent_command, steer_delay):
    ...
    obs = torch.cat([
                        torch.cat((roll, roll)).unsqueeze(-1), #0
                        torch.cat((yaw, yaw)).unsqueeze(-1),  #1
                        torch.cat((d_roll, d_roll)).unsqueeze(-1), #2
                        torch.cat((d_yaw, d_yaw)).unsqueeze(-1), #3
                        torch.cat((speed, speed)).unsqueeze(-1) , #4
                        torch.cat((delta_yaw, delta_yaw)).unsqueeze(-1), #5
                        curent_command.unsqueeze(-1),#6
                        torch.cat((steer_delay, steer_delay)).unsqueeze(-1), #7
                        ], dim=1
                    )
