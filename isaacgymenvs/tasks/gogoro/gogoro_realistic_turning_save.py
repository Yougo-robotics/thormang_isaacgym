...
class Gogoro(VecTask):
    ...
    def generate_spawn_p_r(self):
        ...
        initial_tilt    = 0.0
        initial_heading = (torch.rand(1,device=self.device)*2-1)*np.pi


        x_start =  torch.cos(initial_heading)*100
        y_start =  torch.sin(initial_heading)*100
        z_start = 0.01
    ...
    def post_physics_step(self):
        ...
        lines  = [self.root_positions[test,0].item(), self.root_positions[test,1].item(),self.root_positions[test,2].item()+1.3, 
                  self.root_positions[test,0].item()+torch.cos(self.buffer_obs[test,-1,1]).item()*3, self.root_positions[test,1].item()+torch.sin(self.buffer_obs[test,-1,1]).item()*3,self.root_positions[test,2].item()+1.3]
        colors = [1.0, 0.0, 0.0]
        self.gym.add_lines(self.viewer,self.envs[test],1,lines,colors)

        lines  = [self.root_positions[test,0].item(), self.root_positions[test,1].item(),self.root_positions[test,2].item()+1.3, 
                  self.root_positions[test,0].item()+torch.cos(self.yaw_command[test]).item()*3, self.root_positions[test,1].item()+torch.sin(self.yaw_command[test]).item()*3,self.root_positions[test,2].item()+1.3]
        colors = [0.0, 1.0, 0.0]
        self.gym.add_lines(self.viewer,self.envs[test],1,lines,colors)


        lines  = [0, 0,1, 
                 math.cos(0)*30.0, math.sin(0)*30.0,1]
        colors = [1.0, 0.0, 0.0]
        self.gym.add_lines(self.viewer,self.envs[0],1,lines,colors)


        lines  = [0, 0,1, 
                 math.cos(np.pi)*30.0, math.sin(np.pi)*30.0,1]
        colors = [1.0, 1.0, 0.0]
        self.gym.add_lines(self.viewer,self.envs[0],1,lines,colors)
        ...
        lines  = [0, 0,1, 
                 math.cos(-np.pi/2)*30.0, math.sin(-np.pi/2)*30.0,1]
        colors = [1.0, 1.0, 0.0]
        self.gym.add_lines(self.viewer,self.envs[0],1,lines,colors)
    ...
    def compute_obs_rwd(self):
        ...
        self.speed_no_noise = obs_[:,4]
        #print("speed=",self.speed_no_noise)
    ...
    def reset_idx(self, env_ids):
        ...
        for id in env_ids_int32:
            ...
            if(id == 0):
                self.root_reset_tensor[id,0] = 0.0
                self.root_reset_tensor[id,1] = 0.0
            ...
            self.yaw_command[id] = spawn_yaw + self.get_randoms(1,[-np.pi,np.pi])
...
@torch.jit.script
def compute_gogoro_reward(root_poses,act_buff,curent_obs, progress_buf, max_episode_length, yaw_update_freq):
    tilt      = curent_obs[:, 0]
    yaw_err   = curent_obs[:, 5]


    max_tilt = 1.0
    #reward1 = 1/(1+(yaw_err**2)*100)
    reward1 = torch.exp(-torch.abs(yaw_err)*20)

    prob_actions         = F.softmax(act_buff, dim=1)
    uniform_distribution = torch.ones_like(prob_actions)/prob_actions.shape[1]
    kl_divergence        = torch.sum(prob_actions * torch.log(prob_actions/uniform_distribution), dim=1) * 10.0

    reward3 = -kl_divergence

    print(reward1[0].item())
    print(reward3[0].item())
    print("=====")

    reward  = (reward1 + reward3)/100
    reset   = torch.where(progress_buf > max_episode_length, 1, 0)
    reset   = torch.where(torch.abs(tilt)> max_tilt,1,        reset)

    # time_since_update = progress_buf%yaw_update_freq
    # too_late = torch.logical_and(time_since_update>yaw_update_freq-10,yaw_err>0.1)
    # reset  = torch.where(too_late                 ,1,        reset)
    # reward = torch.where(too_late                 ,reward-0.5,reward)
    reward = torch.where(torch.abs(tilt)> max_tilt,reward-1.0,reward)
    #reward = torch.where(torch.abs(yaw_err)< 0.02,reward+1.0,reward)
